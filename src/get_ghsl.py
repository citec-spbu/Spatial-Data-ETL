...
def download_tile(tile_id: int):
    ...
    logger.error("узнаем статус response")
    response.raise_for_status()
...
def main():
    logger.info(f"starts ghsl downloading")
    download_shapefile()
    get_shapefile() # Shapefile read by geopandas.
    ids = get_tile_ids(top, left, bottom, right)
    srt_spb_ids = _sort_tile_ids(list(ids))
    logger.error(f"{srt_spb_ids}")
    [download_tile(id_) for id_ in srt_spb_ids]
    logger.info(f"")
